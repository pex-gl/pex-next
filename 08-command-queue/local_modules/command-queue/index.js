...
function applyUniforms (ctx, program, uniforms, prevUniforms, prevTextures, debug) {
  if (debug) {
    ...
    for (var requiredUniform in program._uniforms) {
      if (requiredUniform === 'uProjectionMatrix') ctx.setProjectionMatrix(uniforms.uProjectionMatrix)
      if (requiredUniform === 'uViewMatrix') ctx.setViewMatrix(uniforms.uViewMatrix)
      if (requiredUniform === 'uInverseViewMatrix') continue
      if (requiredUniform === 'uModelMatrix') ctx.setModelMatrix(uniforms.uModelMatrix)
  ...
  for (var uniformName in uniforms) {
    ...
    // if (uniformName === 'uProjectionMatrix') ctx.setProjectionMatrix(value)
    // if (uniformName === 'uViewMatrix') ctx.setViewMatrix(value)
    // if (uniformName === 'uInverseViewMatrix') continue
    // if (uniformName === 'uModelMatrix') ctx.setModelMatrix(value)
    // if (uniformName === 'uNormalMatrix') continue
  }
...
CommandQueue.prototype.submit = function (cmd, opts, subCommanDraw) {
  ...
  if (cmd.uniforms && cmd.uniforms.uProjectionMatrix !== undefined) {
    ctx.pushState(ctx.MATRIX_PROJECTION_BIT)
    pushedStates++
    ctx.setProjectionMatrix(cmd.uniforms.uProjectionMatrix)
  }

  if (cmd.uniforms && cmd.uniforms.uViewMatrix !== undefined) {
    ctx.pushState(ctx.MATRIX_VIEW_BIT)
    pushedStates++
    ctx.setViewMatrix(cmd.uniforms.uViewMatrix)
  }

  if (cmd.uniforms && cmd.uniforms.uModelMatrix !== undefined) {
    ctx.pushState(ctx.MATRIX_MODEL_BIT)
    pushedStates++
    ctx.setModelMatrix(cmd.uniforms.uModelMatrix)
  }
  ...
  ctx.pushState(ctx.DEPTH_BIT)
  pushedStates++
  ctx.setDepthTest(cmd.depthTest || true)
  // }
